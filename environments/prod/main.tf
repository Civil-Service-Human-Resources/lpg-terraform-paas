...
module "civil-servant-registry-service" {
  ...
  check_token_url             = "https://identity.cshr.digital/oauth/check_token"
  datasource                  = "jdbc:mysql://${var.rg_prefix}-${var.rg_name}-${var.mysql_name}.mysql.database.azure.com:3306/civil-servant-registry?user=${var.mysql_user}@${var.rg_prefix}-${var.rg_name}-${var.mysql_name}&password=${var.mysql_pass}&useSSL=true&requireSSL=false"
  vaultresourcegroup          = "${var.vaultresourcegroup}"
  vaultname                   = "${var.vaultname}"
  existingkeyvaultsecretname  = "${var.existingkeyvaultsecretname}"
  certificatename             = "${var.certificatename}"
