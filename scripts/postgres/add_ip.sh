...
existing=$(az postgres server firewall-rule list -g $1 -s lpg-$1-postgres | grep -i startipaddress)
...
do
  ...
  if [[ ${existing} != *"${i}"* ]]; then
    ...
    addRule=$(az postgres server firewall-rule create -g $1 -s lpg-$1-postgres --name Rule_${1}_${i//./} --start-ip-address ${i} --end-ip-address ${i})
  fi
done